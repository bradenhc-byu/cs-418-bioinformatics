from C03_EulerianPath.dataset import Dataset
...
if data.contents_loaded:
    print("Total Edges: " + str(data.total_edges))
    e_path = get_eulerian_path(data.graph)
    print("Total path edges: " + str(len(e_path) - 1))
    e_path_str = path_to_string(e_path)
    print(e_path_str)
    outfile = open("./output.txt", "w")
    outfile.write(e_path_str)
    outfile.close()
